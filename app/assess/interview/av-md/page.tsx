...
function calculateResults(responses: Responses) {
  ...
  const arrayVisible      = resp("md2-array-visible");
  const screenedColl      = resp("md2-screened-collections");
  const screenedArray     = resp("md2-screened-array");
  const notVisibleArray   = resp("md2-not-visible-array");
  const verbalDiv1        = resp("md2-verbal-div1");
  const verbalDiv2        = resp("md2-verbal-div2");

  // TG3: Relational thinking
  const comm3x7           = resp("md3-comm-3x7");
  const inverse8x7        = resp("md3-inverse-8x7");
  const dist25x3          = resp("md3-dist-25x3");
  const known8x4          = resp("md3-known-8x4");
  const near15x3          = resp("md3-near-15x3");
  ...
  return {
    mdLevel,
    scores: {
      skipCounting:    { correct: skipCount,                                              total: 6 },
      formGroups:      { correct: formGroups ? 1 : 0,                                    total: 1 },
      arrayTasks:      { correct: [arrayVisible, screenedColl, screenedArray, notVisibleArray].filter(Boolean).length, total: 4 },
      verbalDivision:  { correct: [verbalDiv1, verbalDiv2].filter(Boolean).length,       total: 2 },
      relational:      { correct: relational,                                             total: 5 },
    },
  };
}
...
function InterviewContent() {
    ...
    try {
      ...
      setSavedSessionId(sessionData.id);
        const { error: placementError } = await supabase.from("construct_placements").insert([{
          ...
          student_id:      student.id,
          model_name:      "M&D",
          suggested_level: calc.mdLevel,
          confirmed_level: calc.mdLevel,
          date_placed:     today,
        }]);
  ...
  if (done && student && results) {
    const levelInfo = scheduleAvMD.mdLevels[results.mdLevel];
    const scoreRows: [string, { correct: number; total: number }, number][] = [
      ["Skip Counting",       results.scores.skipCounting,   4],
      ["Forming Groups",      results.scores.formGroups,     1],
      ["Array Tasks",         results.scores.arrayTasks,     3],
      ["Verbal Division",     results.scores.verbalDivision, 1],
      ["Relational Thinking", results.scores.relational,     3],
    ];
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-4">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-500 text-sm mt-1">
                Add+VantageMR Multiplication and Division — {student.first_name} {student.last_name} (Grade {gradeLabel(student.grade_level)})
              </p>
            </div>

            <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3">Suggested Placement</h3>
            <div className="rounded-2xl border-2 border-green-300 bg-green-50 p-4 flex items-center gap-4 mb-6">
              <div className="text-center">
                <div className="text-xs font-bold text-green-600 uppercase tracking-wide mb-1">M&D</div>
                <div className="text-5xl font-black text-green-700">{results.mdLevel}</div>
              </div>
              <div>
                <div className="font-semibold text-green-800 text-sm">{levelInfo?.name}</div>
                <div className="text-green-700 text-xs mt-1 leading-snug">{levelInfo?.description}</div>
              </div>
            </div>
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="h-4 w-px bg-gray-200" />
          <span className="text-gray-500 text-sm">{scheduleAvMD.name}</span>
        </div>
          ...
          <div className="flex gap-1 ml-2">
            {groups.map((_, i) => (
              <button key={i} onClick={() => setCurrentGroupIdx(i)}
                className={`w-3 h-3 rounded-full transition-colors ${i === currentGroupIdx ? "bg-green-600" : i < currentGroupIdx ? "bg-green-200" : "bg-gray-200"}`} />
            ))}
          </div>
        </div>
      </div>
      ...
      {/* Materials banner */}
      {currentGroupIdx === 0 && (
        <div className="bg-green-50 border-b border-green-200 px-4 py-2 text-xs text-green-700 flex items-center gap-2 flex-wrap">
          <span>📦 Materials needed:</span>
          {scheduleAvMD.materials.map((m) => (
            <span key={m} className="bg-green-100 border border-green-300 rounded px-2 py-0.5">{m}</span>
          ))}
        </div>
      )}
          ...
          <div className="flex-1 p-4 flex flex-col gap-4 overflow-y-auto">
            <div className="bg-white border border-gray-200 rounded-xl p-3">
              <div className="mb-3 text-center">
                <div className="text-sm font-bold text-gray-700">Multiplication &amp; Division</div>
                <div className="text-xs text-gray-400 font-medium tracking-wide">Levels 0–5</div>
              </div>
              <div className="space-y-1.5">
                {scheduleAvMD.mdLevels.map(({ level, name, description }) => (
                  <div key={level} className="flex items-start gap-2 rounded-lg px-3 py-2 border bg-gray-50 border-gray-100">
                    <span className="text-xs font-bold text-green-600 w-4 shrink-0 mt-0.5">{level}</span>
                    <div>
...
function SubGroupSection({
  subLevel, items, responses, getResponse, setResponse, startAtItem, startNote, accentColor,
}: {
  subLevel: string;
  ...
  const borderClass = isStartHere ? "border-green-400" : `border-${accent}-200 bg-${accent}-50/20`;

  return (
    <div className={`rounded-xl border-2 overflow-hidden ${isStartHere ? "border-green-400" : "border-green-200 bg-green-50/20"}`}>
      {isStartHere && (
      ...
      <div className="px-3 py-1.5 flex items-center justify-between bg-green-100/60 text-green-900">
        <span className="text-xs font-semibold text-green-800">{subLevel}</span>
        {score && (
    ...
    <div className="px-3 py-3">
      {useMulti ? (
        <div className="flex items-start gap-3">
          {item.displayText && (
            <span className="text-base font-black text-gray-700 min-w-[5rem] text-center shrink-0 pt-0.5">
              {item.displayText}
            </span>
          )}
      ...
      {item.notes && <div className="text-xs text-green-600 mt-0.5 italic">{item.notes}</div>}
    </div>
  );
}
      ...
      {options.map((opt) => (
        <button key={opt} onClick={() => toggle(opt)}
          className={`px-2 py-0.5 rounded text-xs font-medium border transition-all ${
            selected.includes(opt) ? "bg-green-600 border-green-700 text-white" : "bg-white border-gray-300 text-gray-500 hover:border-green-400 hover:text-green-600"
          }`}>
          {opt}
        </button>
      ))}
...
export default function InterviewAvMDPage() {
  return (
